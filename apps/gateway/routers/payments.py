...
_service = PaymentsService(PaymentsRepository({}, {}, {}, {}))
...
@router.post("/confirm")
def confirm(body: IntentConfirmBody):
    try:
...
def refund(body: RefundBody):
    try:
        refund = _service.refund_charge(body.charge_id, body.amount, reason=body.reason)
    except KeyError:
        raise HTTPException(status_code=404, detail="charge_not_found")
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
...
def handle_webhook(envelope: WebhookEnvelope):
    ...
    if payment_state:
        charge_id = event.data.get("data", {}).get("charge_id") or event.data.get("charge_id")
        if charge_id:
            try:
                ...
                pass
    if refund_state:
        refund_id = event.data.get("data", {}).get("refund_id") or event.data.get("refund_id")
        if refund_id:
            try:
